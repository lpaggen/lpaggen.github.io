...
import initChecker, { analyze_ir } from "./checker/pkg/pdc_rust_check.js?v=unsupported-diagnostics-1";
...
async function initializeChecker() {
  const wasm = await fetch("./checker/pkg/pdc_rust_check_bg.wasm?v=unsupported-diagnostics-1");
...
await micropip.install("protobuf==7.35.1")
  ...
  const response = await fetch("./checker/frontend.zip?v=unsupported-diagnostics-1");
  if (!response.ok) {
