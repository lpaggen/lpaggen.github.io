...
import initChecker, { analyze_ir } from "./checker/pkg/pdc_rust_check.js?v=unknown-diagnostics-2";
...
async function initializeChecker() {
  const wasm = await fetch("./checker/pkg/pdc_rust_check_bg.wasm?v=unknown-diagnostics-2");
...
await micropip.install("protobuf==7.35.1")
  ...
  const response = await fetch("./checker/frontend.zip?v=unknown-diagnostics-2");
  if (!response.ok) {
